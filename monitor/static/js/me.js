...
async function loadMeChannelInfo() {
  ...
  channels.forEach(c => {
    if (c.channel_key === 'whatsapp') {
      ME_CHANNEL_INFO[c.channel_key] = { label: c.channel_name, classes: 'text-accent-green bg-accent-green/10 border-accent-green/20', icon: 'bi-whatsapp' };
    } else if (c.channel_key === 'email') {
      ME_CHANNEL_INFO[c.channel_key] = { label: c.channel_name, classes: 'text-blue-400 bg-blue-400/10 border-blue-400/20', icon: 'bi-envelope' };
    } else {
      ME_CHANNEL_INFO[c.channel_key] = { label: c.channel_name, classes: 'text-muted bg-border/30 border-border', icon: 'bi-chat-dots' };
    }
  });
  ME_CHANNEL_INFO.other = { label: 'Outros', classes: 'text-muted bg-border/30 border-border', icon: 'bi-chat-dots' };
}
...
async function renderMeData(range) {
  ...
  document.querySelector('#me-priority-table tbody').innerHTML = orderedPriority.map(r => `
    <tr class="border-b border-border/50 last:border-0 hover:bg-panel-light transition-colors">
      <td class="px-3 py-2.5 truncate w-[130px]">${mePriorityBadge(r.priority)}</td>
  ...
  document.querySelector('#me-channel-table tbody').innerHTML = (detail.by_channel || []).length
    ? detail.by_channel.map(r => {
        const info = ME_CHANNEL_INFO[r.channel] || ME_CHANNEL_INFO.other;
        return `
          <tr class="border-b border-border/50 last:border-0 hover:bg-panel-light transition-colors">
  ...
  document.querySelector('#me-labels-table tbody').innerHTML = (detail.open_labels || []).length
    ? detail.open_labels.map(r => `
        <tr class="border-b border-border/50 last:border-0 hover:bg-panel-light transition-colors">
          <td class="px-4 py-3 truncate max-w-[200px]" title="${r.label}">${meLabelBadge(r.label)}</td>
  ...
  document.querySelector('#me-open-table tbody').innerHTML = status.length
    ? status.map(r => {
        ...
        const chInfo = ME_CHANNEL_INFO[r.channel] || ME_CHANNEL_INFO.other;
        return `
          <tr class="border-b border-border/50 last:border-0 hover:bg-panel-light transition-colors">
            <td class="px-3 py-3 font-semibold text-[0.75rem] text-text whitespace-nowrap">${r.conversation_id}</td>
            <td class="px-3 py-3 truncate">${mePriorityBadge(r.priority)}</td>
            ...
            <td class="px-3 py-3 truncate">${meSlaBadge(r)}</td>
    ...
    : '<tr><td colspan="7"><div class="py-12 text-center flex flex-col items-center"><i class="bi bi-emoji-smile text-[2.5rem] text-accent-green mb-3"></i><span class="text-[1.1rem] text-text font-medium">Caixa limpa!</span><span class="text-[0.9rem] text-muted">Nenhuma conversa aberta.</span></div></td></tr>';
}
    ...
    <div class="flex flex-col gap-6 w-full max-w-[1400px] mx-auto no-scrollbar">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-border">
        <div class="flex items-center gap-4">
          <div id="me-page-avatar" class="w-14 h-14 rounded-xl flex items-center justify-center text-2xl font-bold shrink-0 bg-accent text-white shadow-sm glow-border"></div>
          <div>
            <h2 id="me-page-title" class="m-0 mb-1 text-[1.4rem] font-bold text-text">Meus Dados</h2>
            <p class="m-0 text-[0.9rem] text-muted">Sua visão individual de desempenho</p>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-[1fr_minmax(350px,auto)] gap-6 mb-2">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div class="panel-card bg-panel border border-border rounded-xl p-5 flex flex-col items-center justify-center text-center shadow-sm hover:border-accent-yellow/50 transition-colors group relative overflow-hidden">
            ...
            <span class="text-2xl font-bold text-text leading-none relative z-10" id="me-resolved">-</span>
          </div>
          <div class="panel-card bg-panel border border-border rounded-xl p-5 flex flex-col items-center justify-center text-center shadow-sm hover:border-accent-green/50 transition-colors group relative overflow-hidden">
            ...
            <span class="text-2xl font-bold text-text leading-none relative z-10" id="me-sla">-</span>
          </div>
          <div class="panel-card bg-panel border border-border rounded-xl p-5 flex flex-col items-center justify-center text-center shadow-sm hover:border-accent-blue/50 transition-colors group relative overflow-hidden bg-gradient-to-br from-accent-blue/5 to-panel">
            ...
            <span class="text-2xl font-bold text-text leading-none relative z-10" id="me-awaiting">-</span>
          </div>
          <div class="panel-card bg-panel border border-border rounded-xl p-5 flex flex-col items-center justify-center text-center shadow-sm hover:border-accent-red/50 transition-colors group relative overflow-hidden bg-gradient-to-br from-accent-red/5 to-panel">
            ...
            <span class="text-2xl font-bold text-text leading-none relative z-10" id="me-reopened">-</span>
          </div>
        </div>

        <div id="me-team-compare" class="panel-card hidden relative overflow-hidden bg-panel border border-border rounded-xl shadow-sm p-6 flex-col justify-center min-w-0">
          <div class="absolute -right-6 -top-6 opacity-5 pointer-events-none">
            <i class="bi bi-people-fill text-[12rem]"></i>
          </div>
          <div class="mb-4 z-10">
            <h3 class="m-0 text-base font-semibold text-text flex items-center gap-2"><i class="bi bi-diagram-3 text-muted"></i> Comparativo: <span id="me-team-name" class="text-accent-blue"></span></h3>
          </div>
            ...
            <div class="bg-bg border border-border rounded-xl p-3 flex justify-between items-center shadow-inner">
              <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col">
                  ...
                  <span id="me-frt" class="text-[1.15rem] font-bold text-text leading-none">-</span>
                </div>
              </div>
              <div id="me-cmp-frt"></div>
            </div>
            <div class="bg-bg border border-border rounded-xl p-3 flex justify-between items-center shadow-inner">
              <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col">
                  ...
                  <span id="me-res" class="text-[1.15rem] font-bold text-text leading-none">-</span>
                </div>
              </div>
              <div id="me-cmp-res"></div>
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="panel-card bg-panel border border-border rounded-xl shadow-sm flex flex-col min-w-0 overflow-hidden h-[340px]">
          ...
          <div class="panel-content flex-1 overflow-y-auto relative no-scrollbar">
            <table id="me-priority-table" class="sortable w-full text-left table-fixed">
        ...
        <div class="panel-card bg-panel border border-border rounded-xl shadow-sm flex flex-col min-w-0 overflow-hidden h-[340px]">
          ...
          <div class="panel-content flex-1 overflow-y-auto relative no-scrollbar">
            <table id="me-channel-table" class="sortable w-full text-left table-fixed">
              <thead class="sticky top-0 bg-panel shadow-[0_1px_0_var(--border)] z-10">
                <tr>
        ...
        <div class="panel-card bg-panel border border-border rounded-xl shadow-sm flex flex-col min-w-0 overflow-hidden h-[340px]">
          ...
          <div class="panel-content flex-1 overflow-y-auto relative no-scrollbar">
            <table id="me-subject-table" class="sortable w-full text-left table-fixed">
              <thead class="sticky top-0 bg-panel shadow-[0_1px_0_var(--border)] z-10">
                <tr>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6 mb-6 items-start">
        ...
        <div class="panel-card bg-panel border border-border rounded-xl shadow-sm flex flex-col min-w-0 overflow-hidden h-[380px]">
          <div class="px-4 py-3 border-b border-border flex justify-between items-center cursor-pointer select-none shrink-0" onclick="togglePanel(this)">
            <h3 class="m-0 text-[0.85rem] font-semibold text-text flex items-center gap-2"><i class="bi bi-inbox text-accent-blue glow-text"></i> Minhas Conversas Abertas</h3>
            <button class="text-muted hover:text-text bg-transparent border-none p-1"><i class="bi bi-chevron-up toggle-icon transition-transform"></i></button>
          </div>
          <div class="panel-content flex-1 overflow-x-auto overflow-y-auto relative no-scrollbar">
            <table id="me-open-table" class="sortable w-full text-left table-auto min-w-[700px]">
              <thead class="sticky top-0 bg-panel shadow-[0_1px_0_var(--border)] z-10">
                <tr>
        ...
        <div class="panel-card bg-panel border border-border rounded-xl shadow-sm flex flex-col min-w-0 overflow-hidden h-[380px]">
          ...
          <div class="panel-content flex-1 overflow-y-auto relative no-scrollbar">
            <table id="me-labels-table" class="sortable w-full text-left table-fixed">
              <thead class="sticky top-0 bg-panel shadow-[0_1px_0_var(--border)] z-10">
                <tr>
