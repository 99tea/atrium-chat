...
async def list_tasks(request: Request, user=Depends(get_current_user)):
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT id, content, done, created_at FROM monitor.user_tasks WHERE user_id = $1 ORDER BY done ASC, created_at DESC",
            user["id"],
        )
...
async def create_task(request: Request, user=Depends(get_current_user)):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "INSERT INTO monitor.user_tasks (user_id, content) VALUES ($1,$2) RETURNING id, content, done, created_at",
            user["id"], content,
        )
    return dict(row)
...
async def update_task(task_id: int, request: Request, user=Depends(get_current_user)):
    body = await request.json()
    pool = request.app.state.monitor_pool
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "UPDATE monitor.user_tasks SET done = $1 WHERE id = $2 AND user_id = $3 RETURNING id, content, done, created_at",
            bool(body.get("done", False)), task_id, user["id"],
        )
    if not row:
...
async def delete_task(task_id: int, request: Request, user=Depends(get_current_user)):
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            "DELETE FROM monitor.user_tasks WHERE id = $1 AND user_id = $2",
            task_id, user["id"],
        )
    return {"status": "ok"}
...
async def home_stats(request: Request, user=Depends(get_current_user)):
    pool = request.app.state.monitor_pool
    async with pool.acquire() as conn:
        ...
        rows = await conn.fetch(
            ...
            WHERE e.occurred_at >= date_trunc('day', now() AT TIME ZONE 'America/Sao_Paulo') AT TIME ZONE 'America/Sao_Paulo'
            GROUP BY s.inbox_id
            """
        )
